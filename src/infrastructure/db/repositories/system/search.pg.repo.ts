...
export class SearchPgRepo implements ISearchRepo {
  ...
  async searchStudentsByParentPhone(phone: string, limit: number): Promise<StudentSearchResult[]> {
    // Global search digits spec: match parent_phone only.
    const { rows } = await this.pool.query(
      `${BASE_STUDENT_QUERY} WHERE s.parent_phone = $1 LIMIT $2`,
      [phone, limit]
    );
    return rows.map(mapStudentRow);
  }
