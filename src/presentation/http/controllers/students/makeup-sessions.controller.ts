...
export function createMakeupSessionController(
  ...
  previewMakeupConflictUsecase: any,
) {
  ...
  return {
    ...
    /** GET /makeup-sessions/conflict-preview?makeupDate=&shift=&roomId=&teacherId= */
    previewConflict: async (req: Request, res: Response) => {
      try {
        const result = await previewMakeupConflictUsecase.execute(req.query);
        res.status(200).json(result);
      } catch (error: unknown) {
        sendErrorResponse(res, error);
      }
