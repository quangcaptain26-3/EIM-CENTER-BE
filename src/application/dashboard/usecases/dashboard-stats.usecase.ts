...
export class DashboardStatsUseCase {
  ...
  async execute(userId: string, role: string): Promise<DashboardStatsResult> {
    ...
    if (r === 'ACADEMIC') {
      return this.forAcademic(r);
    }
    if (r === 'ADMIN') {
      return this.forAdmin(r);
    }
    // Role lạ: trả bản hạn chế giống Học vụ để tránh lộ KPI tài chính đầy đủ.
    return this.forAcademic(r);
  }

  /**
   * Học vụ — OVERVIEW 3.2: dashboard tổng quan "Giới hạn" (không biểu đồ doanh thu, không top nợ payroll,
   * không đếm hoàn phí chờ duyệt). Vẫn có tổng học phí chưa thu + lịch + cảnh báo vận hành.
   */
  private async forAcademic(role: string): Promise<DashboardStatsResult> {
    ...
    const [byProg, today, activities, warnings] = await Promise.all([
  ...
  private async fetchAccountantExtras(): Promise<{
    ...
    const [debtors, teachers, byProg, cashAccr] = await Promise.all([
      this.db.query(
        `
        SELECT e.id AS enrollment_id, s.id AS student_id, s.full_name AS student_name, c.class_code,
      ...
      this.db.query(
        ...
        SELECT u.id, u.full_name,
          (
            ...
            WHERE effective_teacher_id(s.id) = u.id AND s.status = 'completed'
  ...
  private async fetchTeacherSlice(userId: string): Promise<TeacherDashboardSlice> {
    ...
    const c = counts[0] as { done: number; upcoming: number };

    const done = Number(c?.done) || 0;
    const upcoming = Number(c?.upcoming) || 0;
    const estimated = done * perSession + allowance;
    const activeClassesRes = await this.db.query(
      `
    ...
    const attendanceRes = await this.db.query(
      ...
      SELECT
        ...
        COUNT(*) FILTER (WHERE a.status = 'absent_excused')::int AS absent_excused_count,
        COUNT(*) FILTER (WHERE a.status = 'absent_unexcused')::int AS absent_unexcused_count
      FROM attendance a
      JOIN sessions s ON s.id = a.session_id
      WHERE effective_teacher_id(s.id) = $1
        AND date_trunc('month', s.session_date::date) = date_trunc('month', CURRENT_DATE)
      `,
      [userId],
    );

    const weekDays = await this.buildTeacherWeek(userId);
    const activeClassRow = activeClassesRes.rows[0] as { total?: number | string } | undefined;
    const attendanceRow = attendanceRes.rows[0] as {
      present_count?: number | string;
      late_count?: number | string;
      absent_excused_count?: number | string;
      absent_unexcused_count?: number | string;
    } | undefined;
    ...
    return {
      ...
      activeClasses: Number(activeClassRow?.total ?? 0),
      attendanceSummary: {
        present: Number(attendanceRow?.present_count ?? 0),
        late: Number(attendanceRow?.late_count ?? 0),
        absentExcused: Number(attendanceRow?.absent_excused_count ?? 0),
        absentUnexcused: Number(attendanceRow?.absent_unexcused_count ?? 0),
      },
      weekDays,
    };
  }
  ...
  private async fetchScheduledWarnings(): Promise<DashboardWarningCard[]> {
    // Các ngưỡng cảnh báo vận hành định kỳ khi vào dashboard.
    // Muốn đổi ngưỡng: sửa số ngày tại query bên dưới (60/30) và cập nhật rules tương ứng.
    const { rows } = await this.db.query(
      `
      WITH pending_refund AS (
        SELECT COUNT(*)::int AS c
        FROM classes c
      ...
      trial_unconfirmed AS (
        SELECT COUNT(*)::int AS c
        FROM enrollments e
        WHERE e.status = 'trial'
          AND COALESCE(e.sessions_attended, 0) >= 2
      )
      SELECT
        pending_refund.c AS pending_refund_count,
        reserved_overdue.c AS reserved_overdue_count,
        trial_unconfirmed.c AS trial_unconfirmed_count
      FROM pending_refund, reserved_overdue, trial_unconfirmed;
      `,
    );

    const row = (rows[0] ?? {}) as Record<string, unknown>;
    const pendingRefundCount = Number(row.pending_refund_count ?? 0);
    const reservedOverdueCount = Number(row.reserved_overdue_count ?? 0);
    const trialUnconfirmedCount = Number(row.trial_unconfirmed_count ?? 0);

    const warnings: DashboardWarningCard[] = [];
    ...
    if (trialUnconfirmedCount > 0) {
      warnings.push({
        code: 'TRIAL_NOT_ACTIVATED_OVER_LIMIT',
        title: 'Học thử chưa xác nhận',
        message: `${trialUnconfirmedCount} ghi danh học thử đã >= 2 buổi nhưng chưa activate.`,
        count: trialUnconfirmedCount,
      });
    }
    return warnings;
  }
