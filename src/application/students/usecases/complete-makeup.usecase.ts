...
export class CompleteMakeupSessionUseCase {
  ...
  async execute(dto: unknown, actor: { id: string; role: string; ip?: string }) {
    ...
    await this.auditLogRepo.log({
      ...
      oldValues: { status: makeupSession.status, sessionsAttended: enrollment.sessionsAttended },
      newValues: { status: 'completed', sessionsAttended: enrollment.sessionsAttended + 1 },
      diff: {
        status: { from: makeupSession.status, to: 'completed' },
        sessionsAttended: { from: enrollment.sessionsAttended, to: enrollment.sessionsAttended + 1 },
      },
