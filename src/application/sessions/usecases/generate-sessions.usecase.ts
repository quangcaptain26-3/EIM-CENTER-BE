...
export class GenerateSessionsUseCase {
  ...
  async execute(userId: string, actorRole: string, classId: string) {
    ...
    const existedSessions = await this.sessionRepo.findByClass(classId);
    if (existedSessions.length > 0) {
      throw new AppError(ERROR_CODES.CLASS_SESSIONS_EXIST, 'Lớp đã có buổi học, không thể sinh lại', 409);
    }
    ...
    // Tự activate enrollment đã thanh toán đủ học phí ngay khi lớp mở.
    await this.db.query(
      `
      UPDATE enrollments e
      SET status = 'active',
          paid_at = COALESCE(e.paid_at, now()),
          updated_at = now()
      WHERE e.class_id = $1
        AND e.status IN ('pending', 'trial')
        AND (
