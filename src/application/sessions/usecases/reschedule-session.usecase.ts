...
export class RescheduleSessionUseCase {
  ...
  async execute(userId: string, actorRole: string, sessionId: string, newDateStr: string, reason: string) {
    ...
    const sameClassSessions = await this.sessionRepo.findByClass(classId);
    const newYmd = newDate.toISOString().slice(0, 10);
    const hasSameDaySession = sameClassSessions.some((item) => {
      if (item.id === sessionId) return false;
      const itemYmd = item.sessionDate.toISOString().slice(0, 10);
      return itemYmd === newYmd && item.status !== 'cancelled';
    });
    if (hasSameDaySession) {
      throw new AppError(
        ERROR_CODES.VALIDATION_ERROR,
        'Lớp đã có buổi học khác vào ngày này',
        409,
      );
    }

    const teacherConflict = await this.conflictChecker.checkTeacherConflictByDateWithDetail({
      teacherId,
      date: newDate,
      shift: session.shift,
    });
    if (teacherConflict.hasConflict) {
      throw new AppError(
        ...
        teacherConflict.conflictReason ?? 'Trùng lịch giáo viên',
        409,
      );
    }

    const isRoomConflict = await this.conflictChecker.checkRoomConflictByDate({
