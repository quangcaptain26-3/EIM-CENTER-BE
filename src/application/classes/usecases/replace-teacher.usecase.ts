...
export class ReplaceTeacherUseCase {
  ...
  async execute(
    ...
    try {
      ...
      if (!minPending) {
        throw new AppError(ERROR_CODES.VALIDATION_ERROR, 'Không còn buổi pending để thay giáo viên', 422);
      }
      fromSessionNo = chosenSessionNo ?? minPending;
      if (fromSessionNo < minPending) {
        throw new AppError(
          ERROR_CODES.VALIDATION_ERROR,
          `Buổi thay giáo viên phải từ buổi pending đầu tiên (#${minPending})`,
          422,
        );
      }
