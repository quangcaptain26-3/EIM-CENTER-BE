...
export class ImportDataUseCase {
  ...
  async execute(type: ImportType, buffer: Buffer, mode: ImportMode, actor: any) {
    ...
    const mappedErrors = (parseResult.errors ?? []).map((e: any) => ({
      row: typeof e?.row === 'number' ? e.row : undefined,
      message: String(e?.reason ?? e?.message ?? 'Lỗi import không xác định'),
      code: typeof e?.field === 'string' ? e.field : undefined,
    }));

    if (mode === 'preview') {
      return {
        success: mappedErrors.length === 0,
    ...
    // All-or-nothing commit: chỉ commit nếu preview không có errors.
