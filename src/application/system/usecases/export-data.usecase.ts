...
} from '../../classes/guards/ensure-class-access-by-role';
  ...
  async execute(type: ExportType, filters: Record<string, unknown>, actor: any): Promise<ExportResult> {
    ...
    switch (type) {
      ...
      case 'payroll': {
        ...
        const teacherId = String(row.teacher_id ?? row.teacherId ?? '');
        if (String(actor.role || '').toUpperCase() === 'TEACHER' && teacherId !== String(actor.id)) {
          throw new AppError(ERROR_CODES.ACCESS_DENIED, 'Chỉ xuất được bảng lương của chính mình', 403);
        }
      ...
      case 'audit-logs': {
        ...
        const dateFromParam = typeof filters.dateFrom === 'string' && filters.dateFrom ? filters.dateFrom : undefined;
        const dateToParam = typeof filters.dateTo === 'string' && filters.dateTo ? filters.dateTo : undefined;

        const fallbackRef = coerceDate(dateToParam ?? today.toISOString().slice(0, 10), today);
        const { from: monthStart, to: monthEnd } = monthRangeEnd(fallbackRef);

        const dateFrom = dateFromParam ? coerceDate(dateFromParam, monthStart) : monthStart;
        const dateTo = dateToParam ? coerceDate(dateToParam, monthEnd) : monthEnd;
        ...
        // Lọc theo domain (AUTH/USER/...) bằng prefix của action: DOMAIN:event
        if (typeof filters.domain === 'string' && filters.domain.trim()) {
          const domain = filters.domain.trim().toUpperCase();
          conditions.push(`al.action LIKE $${idx++}`);
          params.push(`${domain}:`);
        }

        // UI filter theo actorCode (mã snapshot) — cho phép search một phần.
        if (typeof filters.actorCode === 'string' && filters.actorCode.trim()) {
          conditions.push(`al.actor_code ILIKE '%' || $${idx++} || '%'`);
          params.push(filters.actorCode.trim());
        }

        // UI filter theo entityCode (mã nghiệp vụ) — exact match.
        if (typeof filters.entityCode === 'string' && filters.entityCode.trim()) {
          conditions.push(`al.entity_code = $${idx++}`);
          params.push(filters.entityCode.trim());
        }

        // Date range (inclusive)
        conditions.push(`al.event_time::date >= $${idx++}::date`);
        params.push(fromLabel);
        conditions.push(`al.event_time::date <= $${idx++}::date`);
        params.push(toLabel);
